...
3. Proposed block is downloaded by a Taiko node, and the transaction list, as a byte array, is parsed over and checked for validity ([transaction list intrinsic validity function](./proposing#transaction-list-intrinsic-validity-function)).
   - If the transaction list can be RLP decoded into a list of transactions, an ordered subset of the list is created by skipping over transactions which either have an invalid signature or nonce, or the message sender has too little Ether balance to pay for the transaction. This ordered subset is prepended with an [anchor transaction](./proposing#anchor-transaction) to create a Taiko L2 block.
   - Otherwise, an empty block (with only the anchor transaction) is created on L2.
...
On a high level, the proposed block intrinsic validity function checks that the block is valid except for transaction list validity checks. This is separated because on TaikoL1, the transaction list is not known and stored as a data blob. It would also be too expensive to perform these checks on L1.
...
2. The transaction list is no larger than `config.blockMaxTxListBytes`.
3. The sum of all transactions' gas limit is no larger than `config.blockMaxGasLimit`. TODO(Daniel): we need to remove this
4. The total number of transactions is no larger than `config.blockMaxTransactions`.
...
2. The transaction's signature is valid (rule #2 in Ethereum yellow paper). TODO(David): do we check this or skip such a transaction?
3. The transaction's gas limit is no smaller than the intrinsic gas (rule #5 in the Ethereum yellow paper). TODO(David): do we check this or skip such a transaction?
