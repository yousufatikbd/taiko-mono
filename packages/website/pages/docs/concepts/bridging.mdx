...
Taiko's signal service is a smart contract available on both L1 and L2, available for any dapp developer to use. It exposes a [really simple interface](/docs/reference/contract-documentation/signal/ISignalService). And it's this service that makes use of the previously mentioned merkle proofs to provide a service for secure cross-chain messaging.
